...
import { simpleFeatureSEED } from '../fixtures/simple-feature-collection.js'
import { devUser1 } from '../fixtures/users.js'

const RELAY_URL = 'ws://localhost:3334'

const featuresToPublish = [simpleFeatureSEED, multiGeoFeatureCollectionSEED]
...
export async function seedFeatures() {
    ...
    for (const featureCollection of featuresToPublish) {
        try {
            const result = await publishFeatureCollection(ndk, featureCollection.featureCollection, {
                longformNote: featureCollection.note,
            })
